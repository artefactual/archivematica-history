...
public class Fits {
	...
	public static void outputStandardSchemaXml(FitsOutput result, OutputStream out) throws XMLStreamException {
		XmlContent xml = result.getStandardXmlContent();
		if(xml != null) {
			xml.setRoot(true);				
			XMLOutputFactory xmlof = XMLOutputFactory.newInstance();
			XMLStreamWriter writer = xmlof.createXMLStreamWriter(out); 
			xml.output(writer);
		}
		else {
			System.err.println("Error: output cannot be converted to a standard metadata schema for this file");
		}
	}
