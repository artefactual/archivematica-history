...
do
  if [ -d "$FILE" ]; then
    ...
    #Check MD5s
    "$checkMD5NoGui" "/tmp/$UUID/$BASENAME/" "/home/demo/ingestLogs/$UUID/$MD5FILE" "/home/demo/ingestLogs/$UUID/$MD5FILE"processSIP_check.log
    ...
    python "$easyExtract" /tmp/$UUID/ -w -f -r -n 2>&1 >> /home/demo/ingestLogs/$UUID/extraction.log
    ...
    find /tmp/$UUID/ -type f -print| while read NEWDOCS
      do
        chmod 700 $NEWDOCS
        FileUUID=`uuid -v 4`
        "$fileIdentification"  $NEWDOCS $UUID $FileUUID
        echo "$FileUUID -> `"$fileUUIDPY" "Logline" "/tmp/$UUID/$cleanName" "$NEWDOCS"`" >> ~/ingestLogs/$UUID/FileUUIDs.log
    ...
    "$createMETS" "/home/demo/ingestLogs/$UUID" $UUID $cleanName

    # Insert DublinCore.XML into METS.XML
    "$addDublinCoreToMETS" /home/demo/ingestLogs/$UUID /home/demo/ingestLogs/$UUID
