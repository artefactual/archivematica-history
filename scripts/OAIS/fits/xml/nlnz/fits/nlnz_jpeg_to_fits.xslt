...
<xsl:stylesheet version="1.0"
    ...
    <fits xmlns="http://hul.harvard.edu/ois/xml/ns/fits/fits_output"
		...
		<image>
	        ...
	        <YCbCrPositioning>
	        	<xsl:variable name="ycbcrpos" select="JPG/EXIF/YCBCRPOSITIONING/VALUE"/>
	          	<xsl:choose>
					<xsl:when test="$ycbcrpos=1">
						<xsl:value-of select="string('Centered')"/>
					</xsl:when>
					<xsl:when test="$ycbcrpos=2">
              ...
              <samplingFrequencyUnit>
                <xsl:choose>
                  ...
                  <xsl:when test="$units=1">
                    <xsl:value-of select="string('inches')"/>
                  </xsl:when>
                  <xsl:when test="$units=2">
                    <xsl:value-of select="string('centimeters')"/>
                  </xsl:when>
                  <xsl:otherwise>
          ...
          <exposureProgram>
	      	...
	      	<xsl:choose>
				...
				<xsl:when test="$exposureProgram=2">
					<xsl:value-of select="string('Program AE')"/>
				</xsl:when>
				<xsl:when test="$exposureProgram=3">
					<xsl:value-of select="string('Aperture-priority AE')"/>
				</xsl:when>
				<xsl:when test="$exposureProgram=4">
					<xsl:value-of select="string('Shutter speed priority AE')"/>
				</xsl:when>
				<xsl:when test="$exposureProgram=5">
					<xsl:value-of select="string('Creative (Slow speed)')"/>
				</xsl:when>
				<xsl:when test="$exposureProgram=6">
					<xsl:value-of select="string('Action (High speed)')"/>
				</xsl:when>
				<xsl:when test="$exposureProgram=7">
					<xsl:value-of select="string('Portrait')"/>
				</xsl:when>
				<xsl:when test="$exposureProgram=8">
					<xsl:value-of select="string('Landscape')"/>
				</xsl:when>
			</xsl:choose>
	     </exposureProgram>
          ...
          <exifVersion>
          	<xsl:value-of select="JPG/EXIF/EXIFVERSION/VALUE"/>
          </exifVersion>
          ...
          <meteringMode>
	      	...
	      	<xsl:choose>
				...
				<xsl:when test="$meteringMode=4">
					<xsl:value-of select="string('Multi-spot')"/>
				</xsl:when>
				<xsl:when test="$meteringMode=5">
					<xsl:value-of select="string('Multi-segment')"/>
				</xsl:when>
          ...
          <lightSource>
	      	...
	      	<xsl:choose>
				...
				<xsl:when test="$lightSource=3">
					<xsl:value-of select="string('Tungsten')"/>
				</xsl:when>
				...
				<xsl:when test="$lightSource=10">
					<xsl:value-of select="string('Cloudy')"/>
				</xsl:when>
				<xsl:when test="$lightSource=11">
					<xsl:value-of select="string('Shade')"/>
				</xsl:when>
				<xsl:when test="$lightSource=12">
					<xsl:value-of select="string('Daylight Fluorescent')"/>
				</xsl:when>
				<xsl:when test="$lightSource=13">
					<xsl:value-of select="string('Day White Fluorescent')"/>
				</xsl:when>
				<xsl:when test="$lightSource=14">
					<xsl:value-of select="string('Cool White Fluorescent')"/>
				</xsl:when>
				<xsl:when test="$lightSource=15">
					<xsl:value-of select="string('White Fluorescent')"/>
				</xsl:when>
          ...
          <flash>
	      	...
	      	<xsl:choose>
				...
				<xsl:when test="$flash=5">
					<xsl:value-of select="string('Fired, Return not detected')"/>
				</xsl:when>
				<xsl:when test="$flash=7">
					<xsl:value-of select="string('Fired, Return detected')"/>
				</xsl:when>
				<xsl:when test="$flash=8">
					<xsl:value-of select="string('On, Did not fire')"/>
				</xsl:when>
				<xsl:when test="$flash=9">
					<xsl:value-of select="string('On, Fired')"/>
				</xsl:when>
				<xsl:when test="$flash=13">
					<xsl:value-of select="string('On, Return not detected')"/>
				</xsl:when>
				<xsl:when test="$flash=15">
					<xsl:value-of select="string('On, Return detected')"/>
				</xsl:when>
				<xsl:when test="$flash=16">
					<xsl:value-of select="string('Off, Did not fire')"/>
				</xsl:when>
				<xsl:when test="$flash=20">
					<xsl:value-of select="string('Off, Did not fire, Return not detected')"/>
				</xsl:when>
				<xsl:when test="$flash=24">
					<xsl:value-of select="string('Auto, Did not fire')"/>
				</xsl:when>
				<xsl:when test="$flash=25">
					<xsl:value-of select="string('Auto, Fired')"/>
				</xsl:when>
				<xsl:when test="$flash=29">
					<xsl:value-of select="string('Auto, Fired, Return not detected')"/>
				</xsl:when>
				<xsl:when test="$flash=31">
					<xsl:value-of select="string('Auto, Fired, Return detected')"/>
				</xsl:when>
				<xsl:when test="$flash=32">
					<xsl:value-of select="string('No flash function')"/>
				</xsl:when>
				<xsl:when test="$flash=48">
					<xsl:value-of select="string('Off, No flash function')"/>
				</xsl:when>
				<xsl:when test="$flash=65">
					<xsl:value-of select="string('Fired, Red-eye reduction')"/>
				</xsl:when>
				<xsl:when test="$flash=69">
					<xsl:value-of select="string('Fired, Red-eye reduction, Return not detected')"/>
				</xsl:when>
				<xsl:when test="$flash=71">
					<xsl:value-of select="string('Fired, Red-eye reduction, Return detected')"/>
				</xsl:when>
				<xsl:when test="$flash=73">
					<xsl:value-of select="string('On, Red-eye reduction')"/>
				</xsl:when>
				<xsl:when test="$flash=77">
					<xsl:value-of select="string('On, Red-eye reduction, Return not detected')"/>
				</xsl:when>
				<xsl:when test="$flash=79">
					<xsl:value-of select="string('On, Red-eye reduction, Return detected')"/>
				</xsl:when>
				<xsl:when test="$flash=80">
					<xsl:value-of select="string('Off, Red-eye reduction')"/>
				</xsl:when>
				<xsl:when test="$flash=88">
					<xsl:value-of select="string('Auto, Did not fire, Red-eye reduction')"/>
				</xsl:when>
				<xsl:when test="$flash=89">
					<xsl:value-of select="string('Auto, Fired, Red-eye reduction')"/>
				</xsl:when>
				<xsl:when test="$flash=93">
					<xsl:value-of select="string('Auto, Fired, Red-eye reduction, Return not detected')"/>
				</xsl:when>
				<xsl:when test="$flash=95">
					<xsl:value-of select="string('Auto, Fired, Red-eye reduction, Return detected')"/>
				</xsl:when>
			</xsl:choose>
          </flash>
          ...
          <sensingMethod>
	      	...
	      	<xsl:choose>
				...
				<xsl:when test="$sensingMethod=2">
					<xsl:value-of select="string('One-chip color area')"/>
				</xsl:when>
				<xsl:when test="$sensingMethod=3">
					<xsl:value-of select="string('Two-chip color area')"/>
				</xsl:when>
				<xsl:when test="$sensingMethod=4">
					<xsl:value-of select="string('Three-chip color area')"/>
				</xsl:when>
				<xsl:when test="$sensingMethod=5">
					<xsl:value-of select="string('Color sequential area')"/>
				</xsl:when>
				<xsl:when test="$sensingMethod=6">
					<xsl:value-of select="string('Monochrome linear ')"/>
				</xsl:when>
				<xsl:when test="$sensingMethod=7">
					<xsl:value-of select="string('Trilinear')"/>
				</xsl:when>
				<xsl:when test="$sensingMethod=8">
					<xsl:value-of select="string('Color sequential linear')"/>
				</xsl:when>
			</xsl:choose>
	     </sensingMethod>  
