...
if [ -f "${SHA1FILE}" ]; then
    ...
    "`dirname $0`/createXMLeventsMD5Verified.py" "$eventID" "$date" "`sha1deep -v`" "sha1deep" "${target}"  
    ret+="$?"
else
...
if [ -f "${SHA256FILE}" ]; then
    ...
    "`dirname $0`/createXMLeventsMD5Verified.py" "$eventID" "$date" "`sha256deep -v`" "sha256deep" "${target}"  
    ret+="$?"
else
