...
mysql MCP --execute="SELECT FIBE.Extension, CC.classification, CT.TYPE, CR.countAttempts, CR.countOK, CR.countNotOK, CR.countAttempts - CR.countOK + CR.countNotOK AS countIncomplete, Commands.PK AS CommandPK FROM FileIDsByExtension AS FIBE RIGHT OUTER JOIN FileIDs ON FIBE.FileIDs = FileIDs.pk JOIN CommandRelationships AS CR ON FileIDs.pk = CR.FileID JOIN Commands ON CR.command = Commands.pk JOIN CommandClassifications AS CC on CR.commandClassification = CC.pk JOIN CommandTypes AS CT ON Commands.commandType = CT.pk ORDER BY FIBE.Extension, CC.classification" > "Normalization Paths.txt"

mysql MCP --execute="SELECT FIBE.Extension, CC.classification, CT.TYPE, CR.countAttempts, CR.countOK, CR.countNotOK, CR.countAttempts - CR.countOK + CR.countNotOK AS countIncomplete, Commands.PK AS CommandPK, Commands.command FROM FileIDsByExtension AS FIBE RIGHT OUTER JOIN FileIDs ON FIBE.FileIDs = FileIDs.pk JOIN CommandRelationships AS CR ON FileIDs.pk = CR.FileID JOIN Commands ON CR.command = Commands.pk JOIN CommandClassifications AS CC on CR.commandClassification = CC.pk JOIN CommandTypes AS CT ON Commands.commandType = CT.pk ORDER BY FIBE.Extension, CC.classification" > "Normalization Paths With Commands.txt"
