...
def archival_storage(request, path=None):
  ...
  if None != path:
    ...
    if os.path.exists(path):
      ...
      with open(path) as resource:
        response.write(resource.read())
      return response
  # Calculate total archival storage size
  start_path = os.path.dirname(document)
  total_size = 0
  for dirpath, dirnames, filenames in os.walk(start_path):
    for f in filenames:
      fp = os.path.join(dirpath, f)
      total_size += os.path.getsize(fp)
  total_size = '{0:.1f}'.format(total_size / float(1024) / float(1024))
  ...
  for item in tree:
    ...
    sip['size'] = '{0:.2f}'.format(os.path.getsize(os.path.join(os.path.dirname(document), sip['href'])) / float(1024) / float(1024))
    try:
